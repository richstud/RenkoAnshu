...
_DEFAULT_SYMBOLS = [
    ...
    {"id": 7, "symbol": "NZDUSD", "description": "NZD vs USD", "pip_value": 0.0001, "is_active": True},
...
async def get_tickers():
    ...
    2. MT5 symbol list (if connected) — enriches with description/pip info
    3. Hardcoded default list (always works as fallback)
    
    MT5 connectivity is NOT required — tickers always return so the UI is usable.
    """
    ...
    try:
        ...
        if db_data:
            symbol_list = db_data
            logger.info(f"📋 Loaded {len(symbol_list)} symbols from Supabase available_symbols table")
        else:
            # Try MT5 to build list of popular symbols with live info
            import asyncio
            import MetaTrader5 as mt5

            def get_mt5_popular():
                popular_names = [s["symbol"] for s in _DEFAULT_SYMBOLS]
                result = []
                    ...
                    try:
                        ...
                        if info is not None:
                            result.append({
                                ...
                                "pip_value": float(info.point) if info.point else 0.0001,
                                "is_active": True,
                            })
                    except Exception:
                        pass
            ...
            if not symbol_list:
                # Final fallback — hardcoded list, always works
                symbol_list = _DEFAULT_SYMBOLS
                logger.info("📋 Using hardcoded default symbol list (DB empty, MT5 offline)")
            else:
                logger.info(f"📋 Built {len(symbol_list)} symbols from MT5 popular list")
